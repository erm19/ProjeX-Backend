...
export class UserService {
  ...
  async handleUpdatedEvent(userId: string, tenders: Schema.Types.ObjectId[]) {
    // Example logic: Update user in database
    console.log(`Updating user in database: ID=${userId}, tenders=${tenders}`);

    await this._userRepo.update(userId, tenders);
